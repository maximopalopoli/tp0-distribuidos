...
### Ejercicio 7
Los cambios que se implementaron para este ejercicio respecto de la versión anterior son los siguientes:
- Cliente: Una vez que se terminaron de leer archivos (que se identifica por un batch de tamaño 0), se llama al método `finishSendingAndQueryWinners()`, el cual le manda al servidor la clave `WIN`, sumada al agency ID. Esta clave indica que la agencia ya envió toda la información que tenía, y va a quedar en espera de los ganadores. El cliente va a recibir del servidor primero un mensaje indicando el id y la cantidad de ganadores que va a enviar, ambos como verificación. Luego, recibirá a los documentos de los ganadores del sorteo, separados por el caracter `|`, y con un `\n` al final del mensaje. Una vez que termine de recibir los ganadores imprimira un log de éxito y cerrará con código 0.
- Servidor: Cuando reciba en una conexión un mensaje con la clave `WIN`, va a guardar ese socket en un diccionario con el id de la agencia que lo acompaña como clave. Cuando junte 5 claves en el diccionario, entonces va a proceder a realizar el sorteo, comunicando entonces los ganadores a los clientes, que están esperando los resultados. El procedimiento para enviar va a ser, primero filtrar todos los ganadores, luego iterar entre los ids de agencia y por cada id filtrar por dicho id entre los ganadores. En cuanto a la comunicación con el cliente, primero va a enviar el id de la agencia que recibe el mesaje, junto con la cantidad de documentos de ganadores que va a enviar. Luego, va a enviar los documentos de los documentos de los ganadores separados por el caracter `|`, y con un `\n` al final del mensaje (ejemplo: `30876370|24807259\n`). Una vez enviados todos los ganadores, emite un log de éxito y termina el programa.
...
Que cambios se hicieron para manejar las conexiones en paralelo?
- Los métodos `__handle_client_connection` y `receive_data` pasaron a ser funciones para poder ejecutarse en paralelo.
- Tanto el diccionario `active_agencies` como el contador `finished` pasaron a ser manejados a través de un `multiprocessing.Manager`. Esto hizo que ambos recursos compartidos entre los procesos tuvieran un acceso read/write atómico, para evitar race conditions.
